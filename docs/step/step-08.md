...
- [ ] `generate_legal_moves`、`apply_move_unchecked`、`apply_move`、`perft` が共通の内部基盤を利用する構成になっている
- [ ] `make perft-long` が完走する
- [ ] `make perft-long` により、少なくとも mode 1 / mode 2 の深さ 15 既知値一致を確認できる
- [ ] `docs/bench/perft.md` に実行コマンド、測定日、mode、depth、経過時間、備考が記録されている
- [ ] generic 最適化で導入した改善点が README か計測メモで説明されている
- [ ] SIMD を利用した高速化経路が導入されている、または導入しない理由が計測結果とともに説明されている
- [ ] 配布用 `whl` でも runtime dispatch により CPU 機能ごとの実装を自動選択できる構成になっている
- [ ] GitHub Actions で作成した Release artifact を使う場合でも、特定 CPU 固定ではなく runtime dispatch 前提で安全に動く方針が README か計画書に明記されている
