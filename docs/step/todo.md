...
- `Board` と recording で、利用者から見た操作名を可能な限り統一する
- 特に `apply_move` 系は
  - `veloversi.apply_move(board, move)`
  - `veloversi.record_move(record, move)`
  の分裂を見直す
...
- 内部では `Board` の純粋性を維持しつつ、Python 公開面では board 的な操作感を優先する
- この整理は破壊的変更を伴う可能性があるため、`0.2.0` の主要テーマとして扱う
