...
- README は利用者向けに整理済み
- 今後、必要なら次を追加で整備する
  - `Board` / `RecordedBoard` の完全な API 一覧表
  - property を含む公開 API の索引
  - module-level API と method-style API の対応表
...
- examples は実行可能な状態だが、用途ごとに分ける余地がある
- 将来、必要なら次を行う
  - `recording`
  - `dataset`
  - `model_input`
...
- Step 31 では timeout 付き exact 探索のみを公開する
- 後続では、必要なら次を検討する
  - 通常探索 `search_best_move` の Python 公開
  - 探索結果の typed object 化
  - timeout 失敗時の詳細理由や統計情報の拡充

### exact solver 高速化後の追加整理

- Step 32 では exact solver の高速化と root 並列化までを対象にする
- その後、必要なら次を検討する
  - shared TT の導入
  - root parallel の窓共有改善
  - 12 空き超の帯域で実測上有利になる閾値の再調整
  - thread 数や並列化ポリシーの公開設定
  - exact solver 専用 benchmark の整備
  - midgame 探索側への高速化の波及
