...
- Step 28 で `Board` / `RecordedBoard` の core 操作は method-style に揃えた
- 次は symmetry / serialize / feature / model input も method-style に揃える
- batch API は module-level のままでもよいが、単一オブジェクト操作は method-style を主導線に寄せる
