...
- Step 26 では `board` / `recording` からモデル入力を作る正式 API を先に整える
- 推論導線側では、その入力 API を前提にモデル接続を具体化する
- value target の正規化は `final_margin_from_side_to_move / 64.0` を前提に進める

### value-only / policy+value 学習導線

- Step 26 では DataLoader と helper を次の2モードで扱えるようにする
  - `value-only`
  - `policy + value`
- pass 局面は `value-only` では使い、policy 学習では除外する

### history 付き学習 batch

- Step 24 で計画済み
- `moves_until_here` から replay して history を復元する
- `prepare_planes_learning_batch` / `prepare_flat_learning_batch` で `history_len > 0` を扱えるようにする
