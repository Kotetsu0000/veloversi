...
- Step 35 で単体の model 駆動着手選択 API は実装済み
- Step 36 で終盤の exact / model 並列 fallback は実装済み
- Step 38 で PyTorch 学習用 `vv.model.NNUE()` / `export_model(...)` / `load_model(...)` / Rust value model 推論は実装済み
- Step 39 で value 探索の iterative deepening を扱う
- 以後は
  - batch 推論導線
  - typed result
