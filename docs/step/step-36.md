...
この方式だと、`exact_timeout_seconds` が制限時間を大きく消費した場合、model 側に十分な時間が残らない。
...
- exact 側の締切は `min(timeout_seconds, exact_timeout_seconds or timeout_seconds)` にする
- exact がこの個別締切で timeout / failure した時点で、model 結果があれば即返す
...
- [ ] `exact_from_empty_threshold` 以下で exact / model を並列に走らせられる
- [ ] exact が制限時間内に成功した場合は exact を返す
- [ ] exact が未完でも、model 結果があれば model を返せる
- [ ] どちらも結果が無い場合だけ timeout failure になる
...
- exact timeout は `exact_timeout_seconds` を優先し、未指定時は `timeout_seconds`
...
- 解決策:
  - PyO3 wrapper で exact 呼び出しを `allow_threads` 相当にする
...
- 解決策:
  - exact 優先は「個別締切前に exact 成功した場合」に限定する
  - exact が個別締切で timeout / failure した時点で model 結果があれば即返す
  - model 結果が無い場合だけ、全体締切時点まで待つ
