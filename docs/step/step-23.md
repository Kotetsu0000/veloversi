...
- [x] `(B, C, 8, 8)` の feature batch を作れる
- [x] `(B, F)` の feature batch を作れる
- [x] value target batch を作れる
- [x] policy target batch を作れる
- [x] legal move mask batch を作れる
...
- 返却内容
  ...
  - `policy_targets`
  - `legal_move_masks`
- `legal_move_masks` は `(B, 64)`
- `policy_target_index` は `-1`, `0..63`, `64` をそのまま返す
- Step 23 では `history_len == 0` の現局面 batch に限定

## 検証結果

- `make check`: 成功
- Rust: `115 passed; 0 failed; 6 ignored`
- Python: `36 passed`
