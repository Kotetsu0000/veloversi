...
- [x] shared TT が入り、worker 間の探索重複が減っている
- [x] root parallel の窓共有が Step 32 より改善している
- [x] `search_best_move_exact` が少なくとも一部の終盤局面で Step 32 より短時間になる
- [x] timeout 時の失敗結果仕様が維持されている
- [x] exact 結果の正しさが既存テストで維持されている
- [x] `make check` が成功する
...
- `src/search.rs` に shared TT を導入した
- root 並列 exact 探索は 3 段階に分けた
  - `empty < 18`: serial fallback
  - `18 <= empty < 20`: unshared root parallel
  - `empty >= 20`: shared TT を使う root parallel
- timeout 判定は `EXACT_DEADLINE_CHECK_INTERVAL = 65536` として間引き、exact API のオーバーヘッドを削減した
- shared TT は shard 化した `Mutex<FxHashMap<...>>` で実装し、serial fallback では使わない

## 計測結果

Step 32 の benchmark helper と比較した release 計測結果:

- 18 空き:
  - Step 32: `4.247029680s`
  - Step 33: `4.295135670s`
  - 差分: 約 `+1.1%`
  - node 数:
    - Step 32: `62762867`
    - Step 33: `62762867`
- 20 空き:
  - Step 32: `6.136336929s`
  - Step 33: `6.035779161s`
  - 差分: 約 `-1.6%`
  - node 数:
    - Step 32: `106833467`
    - Step 33: `101407875`
...
- 18 空きでは shared TT の競合が不利なので、Step 33 では unshared root parallel に戻した
- 20 空きでは shared TT と worker pool が有効で、Step 32 より短時間・少ない node 数になった
- 受け入れ条件である「少なくとも一部の終盤局面で Step 32 より短時間になる」は満たしている

## 検証結果

- `cargo test search::tests -- --nocapture`: 成功
- `cargo test --release exact_search_bench_step32_vs_step33_ -- --ignored --nocapture`: 成功
- `make check`: 成功
  - Rust: `160 passed; 0 failed; 13 ignored`
  - Python: `60 passed`
