use crate::flip_tables::{
    BB_DLINE02, BB_DLINE57, BB_FLIPPED, BB_H2VLINE, BB_MUL16, BB_SEED, BB_VLINE,
};
...
mod tests {
    ...
    fn print_simd_status() {
        println!("VELOVERSI_SIMD={:?}", super::simd_preference());
        println!("movegen backend={}", super::selected_movegen_backend());
        println!("flip backend={}", super::selected_flip_backend());
        println!("board backend={}", super::selected_board_backend());
    }
    ...
    fn resolve_backend_selection_matches_documented_matrix() {
        assert_eq!(
            super::resolve_movegen_backend(SimdPreference::Generic, false),
            MovegenBackend::Generic
        );
        assert_eq!(
            super::resolve_movegen_backend(SimdPreference::Sse2, false),
            MovegenBackend::Generic
        );
        assert_eq!(
            super::resolve_movegen_backend(SimdPreference::Auto, false),
            MovegenBackend::Generic
        );
        assert_eq!(
            super::resolve_movegen_backend(SimdPreference::Auto, true),
            MovegenBackend::Avx2
        );
        assert_eq!(
            super::resolve_board_backend(SimdPreference::Generic),
            BoardBackend::Generic
        );
        assert_eq!(
            super::resolve_board_backend(SimdPreference::Sse2),
            BoardBackend::Sse2
        );
        assert_eq!(
            super::resolve_board_backend(SimdPreference::Avx2),
            BoardBackend::Sse2
        );
        assert_eq!(
            super::resolve_board_backend(SimdPreference::Auto),
            BoardBackend::Sse2
        );
        assert_eq!(
            super::resolve_flip_backend(SimdPreference::Generic, false),
            FlipBackend::Generic
        );
        assert_eq!(
            super::resolve_flip_backend(SimdPreference::Sse2, false),
            FlipBackend::Generic
        );
        assert_eq!(
            super::resolve_flip_backend(SimdPreference::Auto, false),
            FlipBackend::Generic
        );
        assert_eq!(
            super::resolve_flip_backend(SimdPreference::Auto, true),
            FlipBackend::Avx2
        );
    }
    ...
    fn forcing_avx2_without_cpu_support_panics_in_backend_resolution() {
        let movegen = std::panic::catch_unwind(|| {
            super::resolve_movegen_backend(SimdPreference::Avx2, false)
        });
        let flip =
            std::panic::catch_unwind(|| super::resolve_flip_backend(SimdPreference::Avx2, false));
    ...
    fn avx2_flip_matches_generic_oracle_for_curated_positions() {
        ...
        for board in positions {
            for mv in legal_move_list(generate_legal_moves_naive(&board)) {
                ...
                let generic = super::flips_for_move_bits_unchecked_generic(
                    player_bits,
                    opponent_bits,
                    mv.square,
                );
                let avx2 = unsafe {
                    super::flips_for_move_bits_avx2(player_bits, opponent_bits, mv.square)
                };
    ...
    fn selected_backends_match_runtime_resolution() {
        let preference = super::simd_preference();
        let has_avx2 = std::arch::is_x86_feature_detected!("avx2");

        assert_eq!(
            super::selected_movegen_backend_kind(),
            super::resolve_movegen_backend(preference, has_avx2)
        );
        assert_eq!(
            super::selected_board_backend_kind(),
            super::resolve_board_backend(preference)
        );
        assert_eq!(
            super::selected_flip_backend_kind(),
            super::resolve_flip_backend(preference, has_avx2)
        );
    }
