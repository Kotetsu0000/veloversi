...
説明:
- 終局局面を前提として `black_count - white_count` を返す
...
説明:
- 終局局面を前提として「その局面の `side_to_move` 視点」の石差を返す
- 強制パス後の視点整合に注意し、定義は関数コメントに明記する
...
```python
def initial_board() -> PyBoard: ...
def board_from_bits(black_bits: int, white_bits: int, side_to_move: str) -> PyBoard: ...
def validate_board(board: PyBoard) -> None: ...
def generate_legal_moves(board: PyBoard) -> int: ...
def legal_moves_list(board: PyBoard) -> list[int]: ...
def is_legal_move(board: PyBoard, square: int) -> bool: ...
def apply_move(board: PyBoard, square: int) -> PyBoard: ...
def apply_forced_pass(board: PyBoard) -> PyBoard: ...
def board_status(board: PyBoard) -> str: ...
def disc_count(board: PyBoard) -> tuple[int, int, int]: ...
def game_result(board: PyBoard) -> str: ...
def final_margin_from_black(board: PyBoard) -> int: ...
def encode_planes(board: PyBoard, history: list[PyBoard], config: dict) -> numpy.ndarray: ...
def encode_nnue_features(board: PyBoard) -> tuple[numpy.ndarray, numpy.ndarray]: ...
def transform_board(board: PyBoard, sym: str) -> PyBoard: ...
