...
```bash
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0-cp312-abi3-manylinux_2_34_x86_64.whl"
```

### Linux aarch64

```bash
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0-cp312-abi3-manylinux_2_34_aarch64.whl"
```

### macOS Intel

```bash
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0-cp312-abi3-macosx_10_12_x86_64.whl"
```

### macOS Apple Silicon

```bash
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0-cp312-abi3-macosx_11_0_arm64.whl"
```

### Windows x86_64

```powershell
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0-cp312-abi3-win_amd64.whl"
```
...
```bash
uv add "https://github.com/Kotetsu0000/veloversi/releases/download/v0.5.0/veloversi-0.5.0.tar.gz"
```
...
value model で序盤局面を評価し、model-filtered balanced openings を JSONL に保存できます。
絶対的な互角局面を保証するものではなく、結果は model と探索深さに依存します。
...
for item in vv.iter_opening_boards(openings):
    sequence = item["sequence"]
    board = item["board"]
```

`value_scale="normalized"` では `tanh(raw_value)` を閾値判定に使います。
`value_scale="raw"` では model raw output をそのまま閾値判定に使います。raw scale は model や `eval_mode` に依存する advanced option です。

候補局面そのものの静的評価ではなく、数手先読みした root value で判定したい場合は `eval_mode="value_search"` を使います。

```python
stats = vv.generate_balanced_opening_file(
    model,
    "balanced-openings-search.jsonl",
    plies=8,
    threshold=2 / 64,
    value_scale="normalized",
...
`prefilter_threshold` は高速化用です。静的評価で候補を落とすため、精度優先なら `None` を指定します。
PyTorch model を GPU で使う場合、初版の value search は leaf を 1 局面ずつ評価するため遅くなることがあります。
value model の出力は現在手番視点である必要があります。
`plies=8` は慣例的な既定値です。別の値では保存される `side_to_move` も変わります。
board だけ必要なら `openings.boards`、sequence や value metadata も処理するなら `iter_opening_boards(...)` を使います。
